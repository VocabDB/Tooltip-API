...
{
	global $G_path_dn_image,$G_path_audio_word;
		...
		for ($i=0; $i<$max_word;$i++) { 
			if ($word[$i]['voca'] <> '') {
				...
				$audio_word = $G_path_audio_word.addslashes(str_replace(" ","%20",$word[$i]['voca']));
				if ($cnt ==1) {
...
function print_idiomlist_PC($word,$cnt,$tlang) 
{
		...
		for ($i=0; $i<$max_idiom;$i++) { 
			if (isset($word[$i]['idiom']) and $word[$i]['idiom'] <> '') {
				if ($k == 1) { $bg_color = 'white'; $k =0; } else { $bg_color = '#F6F6F6'; $k =1;}
				$audio_word = $G_path_audio_word.addslashes(str_replace(" ","%20",$word[$i]['idiom']));
				if ($cnt ==1) {
