...
function print_wordlist_PC($word,$cnt,$tlang) {
		...
		for ($i=0; $i<$max_word;$i++) { 
			if ($word[$i]['voca'] <> '') {
				...
				$audio_word = addslashes(str_replace(" ","%20",$word[$i]['voca']));
				if ($cnt ==1) {
					$dict .=fill_space($word[$i]['voca']).' '.$word[$i]['means'].' ('.$word[$i]['part'].' - '.$word[$i]['level'].")&nbsp;&nbsp;";
				} else { 

					$dict .="<tr height='50px' bgcolor='$bg_color'><td style='width:20%; padding-left:10px;'><span class='word_circle'></span><a href=\"javascript:audio_play('$audio_word',0)\">";
					$dict .="<span class='ajax_list_word'>".$word[$i]['voca']."</a></td><td width='60%'><span class='ajax_list_means'>";
...
/* 					if ($word[$i]['means'] <> '')  {
						$dict .= "<span class='tooltip_dn'><img onclick='save_word(this,1)' src='images/dn_won.png' /></span>";
					} 
*/
					$dict .= "</span></td></tr>";
		...
		for ($i=0; $i<$max_word;$i++) { 
			if ($word[$i]['voca'] <> '') {
				...
				if ($cnt ==1) {
					...
					$dict .="<tr height='50px' bgcolor='$bg_color'><td style='width:100%; padding-left:10px;'><span class='word_circle'></span><a href=\"javascript:audio_play('$audio_word',0)\">";
					$dict .="<span class='ajax_list_word'>".$word[$i]['voca']."</span></a>&nbsp;&nbsp;";
					$dict .="<small>-&nbsp;&nbsp;".$word[$i]['part']." (".$word[$i]['level'].")</small><br />&nbsp;";
					$dict .="<span class='ajax_list_means'>";
					$meaning = $word[$i]['means'];
					if ($tlang=='ar' or $tlang=='fa') {
						$meaning = disp_means_arabic($word[$i]['means']);
					} else {
...
function print_idiomlist_PC($word,$cnt,$tlang) {
		...
		for ($i=0; $i<$max_idiom;$i++) { 
			if (isset($word[$i]['idiom']) and $word[$i]['idiom'] <> '') {
				if ($k == 1) { $bg_color = 'white'; $k =0; } else { $bg_color = '#F6F6F6'; $k =1;}
				$audio_word = addslashes(str_replace(" ","%20",$word[$i]['idiom']));
				if ($cnt ==1) {
					$tra .=fill_space($word[$i]['idiom']).' '.$word[$i]['means'].' ('.$word[$i]['level'].")&nbsp;&nbsp;";	
				} else {

					$tra .="<tr height='50px' bgcolor='$bg_color'><td width='30%'>&nbsp;&nbsp;<span class='word_circle'></span>&nbsp;<a href=\"javascript:audio_play('$audio_word',0)\">";
					$tra .="<span class='ajax_list_word'>".$word[$i]['idiom']."</a></td><td width='50%'>";
...
/* 					if ($word[$i]['means'] <> '')  {
						$tra .= "<span class='tooltip_dn'><img onclick='save_word(this,1)' src='images/dn_won.png' /></span>"; 
					}	
*/
					$tra .= "</span></td></tr>";					
		...
		for ($i=0; $i<$max_idiom;$i++) { 
			if ($word[$i]['idiom'] <> '') {
				...
				if ($cnt ==1) {
					...
					$tra .="<tr height='50px' bgcolor='$bg_color'><td style='width:100%; padding-left:10px;'><span class='word_circle'></span><a href=\"javascript:audio_play('$audio_word',0)\">";
...
// Don't use it.
function print_result_api_simple($api_array,$word,$cnt,$tlang)  
{
		$word = $api_array['contents']['words']['relative_words'];
		$max_word = sizeof($word);
		$dict = '';
		if ($word <> '') {
			for ($i=0; $i<$max_word;$i++) { 
				if ($word[$i]['voca'] <> '') {
					if ($cnt ==1) {
						...
						$dict .="<tr><td width='20%'>".$word[$i]['voca']."</td><td width='60%'>";
							if ($tlang=='ar' or $tlang=='fa') {
								$meaning = disp_means_arabic($word[$i]['means']);
							} else {
								$meaning = $word[$i]['means'];
							}
						$dict .= $meaning."</td><td width='20%'>".$word[$i]['part']." - ".$word[$i]['level']."</td></tr>";
					}
				}
			}
		}
		if ($dict!='') { 
			$color ='#fff';
			if ($cnt ==1)	{
				$dict ="<div class='ajax_table_word'>".$dict."</div>";
			} else {
				$dict ="<div class='ajax_table_word'><table width='100%; color:#fff;'><tbody>".$dict."</table></tbody></div>";
			}
		 }
		$word = $api_array['contents']['idioms']['relative_idioms'];
		$max_idiom = sizeof($word);
		$tra='';
		...
		if ($word <> '') {
			for ($i=0; $i<$max_idiom;$i++) { 
				if ($word[$i]['idiom'] <> '') {
					if ($cnt ==1) {
						$tra .=fill_space($word[$i]['idiom']).' ';
						if ($tlang=='ar' or $tlang=='fa') {
							$meaning = disp_means_arabic($word[$i]['means']);
						} else {
							$meaning = $word[$i]['means'];
						}
						$tra .= $meaning.' ('.$word[$i]['level'].")&nbsp;&nbsp;";
					} else {
						if ($word[$i]['idiom'] <> '') {
							$tra .="<tr><td width='20%'>".$word[$i]['idiom']."</td><td width='60%'>";
							if ($tlang=='ar' or $tlang=='fa') {
								$meaning = disp_means_arabic($word[$i]['means']);
							} else {
								$meaning = $word[$i]['means'];
							}
							$tra .= $meaning.$word[$i]['means']."</td><td width='20%'>".$word[$i]['level']."</td></tr>";
						}
					}
				}
			}
		...
		if ($tra !='') {
			$color ='#c7def9';
			if ($cnt ==1) {
				$tra ="<div class='ajax_table_idiom'>".$tra."</div>";
			} else {  
				$tra ="<div class='ajax_table_idiom'><table width='100%'><tbody>".$tra."</tbody></table></div>";
			}
		}
		return $dict.$tra;
}
...
{
		$trans = "<div class='ajax_trans_site'><a onclick=\"tooltip_trans_site(0,'".$slang."','".$tlang."','".addslashes($vExam)."')\" style='cursor:pointer;'><img src='http://vocabdb.com/images/t_ms_m.png' title='MS Bing'></a><a onclick=\"tooltip_trans_site(1,'".$slang."','".$tlang."','".addslashes($vExam)."')\" style='cursor:pointer;'><img src='http://vocabdb.com/images/t_g_m.png' title='Google'></a></div>";
	return $trans;
}
	...
	if (!$result_api) {
		...
		if ($simple_print) {
			...
			$api_text = print_result_api_simple($result,$word,$wordcount,$tlang); 
			$result='';
